...
### Projects
**Assertico** | *Next.js, TypeScript, PostgreSQL, Prisma ORM, Supabase Auth, Tailwind CSS, shadcn/ui*
- Architected a full-stack API testing platform integrated with a custom assertion engine, a dynamic request builder, and a response diff checker, reducing API validation and debugging cycles by **50%**.
- Engineered a secure request history management system with persistent storage using Supabase, PostgreSQL, and Prisma ORM, improving dashboard data retrieval speeds by **220ms** via strategic query optimization.

**PrepMetrics** | *MERN Stack*
- Developed a scalable quiz and performance analytics platform that automates subject-wise assessments, delivering real-time accuracy metrics and granular data insights for **100+ active users**.
- Optimized complex database queries and designed interactive analytics dashboards, cutting data retrieval latency by **40%** across high-volume, multi-subject student datasets.

**ScanNDine** | *MERN Stack*
- Developed a full-stack restaurant ordering ecosystem featuring automated table-specific QR code generation to digitize menu accessibility, reducing manual ordering overhead by **60%**.
- Implemented secure JWT authentication, role-based admin/staff controls, and optimized MongoDB schemas, boosting overall operational efficiency and system scalability by **40%**.
